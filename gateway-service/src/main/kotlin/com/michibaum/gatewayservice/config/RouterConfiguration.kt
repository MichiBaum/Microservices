...
class RouterConfiguration {
    ...
    fun gatewayRoutes(
        ...
        return route()
            ...
            .route(hostHttp2Aware("admin.michibaum.*"), lb("admin-service").apply(http()))
            .route(hostHttp2Aware("registry.michibaum.*"), lb("registry-service").apply(http()))
            .route(hostHttp2Aware("authentication.michibaum.*"), lb("authentication-service").apply(http()))
            .route(hostHttp2Aware("usermanagement.michibaum.*"), lb("usermanagement-service").apply(http()))
            .route(hostHttp2Aware("chess.michibaum.*"), lb("chess-service").apply(http()))
            .route(hostHttp2Aware("fitness.michibaum.*"), lb("fitness-service").apply(http()))
            .route(hostHttp2Aware("music.michibaum.*"), lb("music-service").apply(http()))
