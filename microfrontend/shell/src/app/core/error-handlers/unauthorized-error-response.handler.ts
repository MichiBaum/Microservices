...
@Injectable()
export class UnauthorizedErrorResponseHandler implements IHttpErrorResponseHandler {
  ...
  private generateMessage(error: HttpErrorResponse): Message {
    if (environment.show_errors === true) {
      ...
      return LogGenerator.createToastError(
        {
          name: this.translate.instant('error.401.name'),
          message: this.translate.instant('error.401.message')
        } as Error
      );
    }
  }
