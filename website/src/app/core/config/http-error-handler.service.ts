...
export class HttpErrorHandler {
  ...
  handleError(error: HttpErrorResponse, userInfoService: UserInfoService, customErrorMatcher: CustomErrorMatching | undefined = undefined) { // TODO if userInfoService is injected via constructor it is undefined. I have tried providing in app.config
    let errorSummary: string;
    let errorMessage: string;

    const customCondition = customErrorMatcher?.matcher(error) ?? false;
